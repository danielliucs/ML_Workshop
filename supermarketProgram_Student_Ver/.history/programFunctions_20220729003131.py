...
def viewInventory(): # declaring function
    for food, info in inventory.items(): # for loop: items() returns "name of food", and "all its information" in 'food' and 'info' (in dict. data structure) respectivly
        print("\nFood Type:", food) # displays the food's name
        for key in info: # for all the keys in the info dictionary
            print(key + ':', info[key]) # print the key name and its value 
    print("\n") # formatting

def removeItem(key): # declaring function
    if key in inventory.keys(): # keys() returns list of all keys in the dictionary --> if 'key' needed is in that list do below
        inventory.pop(key) # pop() removes the 'key' from the inventory dictionary
        viewInventory() # call viewInventory to print the updated inventory
    else: # if key doesn't exist
        print("This item doesn't exist.\n") # print message in terminal 

def addItem(): # declaring function
    ...
    inventory[item] = {} # new dictionary
    inventory[item]['price'] = float(input("How much does it cost? ")) # for that item, get its price

    stock = str(input("Is it stock? (type yes/no) ")).lower() # get its stock and change user response to lowercase 
    if stock == "yes": # lowercase used because of lower() module
        stock = True # boolean data type
    else:
        stock = False # boolean data type
    inventory[item]['inStock'] = stock # add key 'inStock' and set its value to bool(stock)

    discount = str(input("Enter the percentage discount. ")).lower() # prompt user and get discount
    if discount[-1] == "%": # if the % was entered, input that new entery
        inventory[item]['discount'] = discount
    else:
        inventory[item]['discount'] = discount + "%" # add '%' sign if the user didn't add it
